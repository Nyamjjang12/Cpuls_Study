...
enum EMonth {
	...
	Decemver,
	/**
	열거형의 요소는 전역처럼 작동하기 떄문에 서로 다른 enum 영역이라도 변수명이 같으면 오류가 납니다.
	같은 영역내에서 동일한 변수 선언 불가능합니다.
	*/
	// Max, // (X)
	MaxMonth,
};
...
테스트를 위해서 EWeek라는 이름의 열겨형을 정의해 줍니다.
*/
enum EWeek {
	Sunday =0,
	Monday,
...
int main()
{
	...
	*/
	...
	// ERainbowColor rainbowColor = Red; // 이름공간이 없으니 에러가 생깁니다.
	...
	*/
	...
	for (int i = 0; i < static_cast<int>(ERainbowColor::MaxRainbowColor); i++) {
		...
		cout << "color : " << value << ", name : " << name << endl;
	}
	cout << "" << endl;
	cout << "" << endl;
}