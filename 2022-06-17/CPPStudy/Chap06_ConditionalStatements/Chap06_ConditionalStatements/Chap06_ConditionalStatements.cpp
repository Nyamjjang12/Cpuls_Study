...
int main()
{
    ...
    */
    ...
    {
        // 위의 조건식에 아무 것도 해당되지 않을 떄 실행될 문장
    }
    */
    if (intValue1 > intValue2) {
        cout << intValue1 << " is greate than " << intValue2 << endl;
    }
    else {
        cout << intValue1 << " is less than " << intValue2 << endl;
    }
    ...
    */
    ...
    if문을다른 if 문 안에 중첩할 수 있습니다.
    헷갈릴 수 있습니다. 않좋은 습관입니다.
    */
    if (intValue1 >= 10)
    {
        if (intValue1 <= 20) {
            cout << intValue1 << " is vetween 10 and 20 " << endl;
        }
        else {
    ...
    하지만 switch문 if문 처럼 <, <=, >, >= 와 같은 이상, 이하, 미만, 초과와 같은 부등식이 사용될 수 없습니다.
    ...
    하지만 whitch문을 사용함으로꺼 if문 모다는 좀 더 코드의 가독성이 좋게 작성할 수 있고 if문 보다 미세하게
    switch문이 빠르기에 switch문으로 작성할 수 있는 코드가 있다고 하면 if문보다 switch문이 좋습니다.

    switch문을 쓸 땨 주의할 점은 다음과 같습니다.
    ...
    */
    ...
    string greaterMessage1 = intValue1 + " is greater than " + intValue2;
